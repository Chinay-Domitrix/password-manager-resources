...
function _markBitsForNamedCharacterClass(bitSet, namedCharacterClass) {
	...
	if (namedCharacterClass.name === Identifier.UPPER) bitSet.fill(true, _bitSetIndexForCharacter("A"), _bitSetIndexForCharacter("Z") + 1); else if (namedCharacterClass.name === Identifier.LOWER) bitSet.fill(true, _bitSetIndexForCharacter("a"), _bitSetIndexForCharacter("z") + 1); else if (namedCharacterClass.name === Identifier.DIGIT) bitSet.fill(true, _bitSetIndexForCharacter("0"), _bitSetIndexForCharacter("9") + 1); else if (namedCharacterClass.name === Identifier.SPECIAL) {
		...
		bitSet.fill(true, _bitSetIndexForCharacter("["), _bitSetIndexForCharacter("`") + 1);
		bitSet.fill(true, _bitSetIndexForCharacter("{"), _bitSetIndexForCharacter("~") + 1);
	} else console.assert(false, SHOULD_NOT_BE_REACHED, namedCharacterClass);
}

function _markBitsForCustomCharacterClass(bitSet, customCharacterClass) {
	for (let character of customCharacterClass.characters) bitSet[_bitSetIndexForCharacter(character)] = true;
}
...
function _canonicalizedPropertyValues(propertyValues, keepCustomCharacterClassFormatCompliant) {
	...
	for (let propertyValue of propertyValues) if (propertyValue instanceof NamedCharacterClass) {
		if (propertyValue.name === Identifier.UNICODE) return [new NamedCharacterClass(Identifier.UNICODE)];
		if (propertyValue.name === Identifier.ASCII_PRINTABLE) return [new NamedCharacterClass(Identifier.ASCII_PRINTABLE)];
		_markBitsForNamedCharacterClass(asciiPrintableBitSet, propertyValue);
	} else if (propertyValue instanceof CustomCharacterClass) _markBitsForCustomCharacterClass(asciiPrintableBitSet, propertyValue);
	let charactersSeen = [];
	const checkRange = (start, end) => {
		let temp = [];
		for (let i = _bitSetIndexForCharacter(start); i <= _bitSetIndexForCharacter(end); ++i) if (asciiPrintableBitSet[i]) temp.push(_characterAtBitSetIndex(i));
		let result = temp.length === _bitSetIndexForCharacter(end) - _bitSetIndexForCharacter(start) + 1;
		if (!result) charactersSeen = charactersSeen.concat(temp);
		return result;
	};
	...
	for (let i = _bitSetIndexForCharacter(" "); i <= _bitSetIndexForCharacter("/"); ++i) {
		if (!asciiPrintableBitSet[i]) continue;

		let character = _characterAtBitSetIndex(i);
		if (keepCustomCharacterClassFormatCompliant && character === "-") hasDash = true; else temp.push(character);
	}
	for (let i = _bitSetIndexForCharacter(":"); i <= _bitSetIndexForCharacter("@"); ++i) if (asciiPrintableBitSet[i]) temp.push(_characterAtBitSetIndex(i));
	for (let i = _bitSetIndexForCharacter("["); i <= _bitSetIndexForCharacter("`"); ++i) {
		if (!asciiPrintableBitSet[i]) continue;
		let character = _characterAtBitSetIndex(i);
		if (keepCustomCharacterClassFormatCompliant && character === "]") hasRightSquareBracket = true; else temp.push(character);
	}
	for (let i = _bitSetIndexForCharacter("{"); i <= _bitSetIndexForCharacter("~"); ++i) if (asciiPrintableBitSet[i]) temp.push(_characterAtBitSetIndex(i));
	if (hasDash) temp.unshift("-");
	if (hasRightSquareBracket) temp.push("]");
	...
	if (!hasAllSpecial) charactersSeen = charactersSeen.concat(temp);
	let result = [];
	if (hasAllUpper && hasAllLower && hasAllDigits && hasAllSpecial) return [new NamedCharacterClass(Identifier.ASCII_PRINTABLE)];
...
function _parseIdentifier(input, position) {
	...
	do {
		...
		if (!_isIdentifierCharacter(c)) break;
...
function _parseCustomCharacterClass(input, position) {
	...
	do {
		...
		result.push(c);
		++position;
		if (c === CHARACTER_CLASS_END_SENTINEL) break;
	} while (position < length);
	...
	if (position < length && input[position] === CHARACTER_CLASS_END_SENTINEL) return [result, position + 1];
...
function _parsePasswordRequiredOrAllowedPropertyValue(input, position) {
	...
	while (true) {
		if (_isIdentifierCharacter(input[position])) {
			...
			if (propertyValue && propertyValue.length) propertyValues.push(new CustomCharacterClass(propertyValue));
		} else {
		...
		if (position >= length || input[position] === PROPERTY_SEPARATOR) break;
...
function _parsePasswordRule(input, position) {
	...
	position = _indexOfNonWhitespaceCharacter(input, position + 1);
	// Empty value
	if (position >= length || input[position] === PROPERTY_SEPARATOR) return [new Rule(property.name, property.value),
	                                                                          position];
	switch (identifier) {
		case RuleName.ALLOWED:
		case RuleName.REQUIRED: {
			var [propertyValue, position] = _parsePasswordRequiredOrAllowedPropertyValue(input, position);
			if (propertyValue) property.value = propertyValue;
...
function _parseInteger(input, position) {
	...
	if (!_isASCIIDigit(input[position])) {
		console.error(`Failed to parse value of type integer; not a number: ${input.substring(position)}`);
		return [null, position];
	}
	let length = input.length;
	let initialPosition = position;
	...
	if (position >= length || input[position] === PROPERTY_SEPARATOR) return [result, position];
	console.error(`Failed to parse value of type integer; not a number: ${input.substring(initialPosition)}`);
	return [null, position];
}
...
function _parsePasswordRulesInternal(input) {
	...
	while (position < length) {
		if (!_isIdentifierCharacter(input[position])) {
			console.warn(`Failed to find start of property: ${input.substring(position)}`);
			return parsedProperties;
		}
		var [parsedProperty, position] = _parsePasswordRule(input, position);
		if (parsedProperty && parsedProperty.value) parsedProperties.push(parsedProperty);
		position = _indexOfNonWhitespaceCharacter(input, position);
...
function parsePasswordRules(input, formatRulesForMinifiedVersion) {
	...
	for (let rule of passwordRules) switch (rule.name) {
		case RuleName.MAX_CONSECUTIVE:
		...
		case RuleName.REQUIRED:
			...
			if (!suppressCopyingRequiredToAllowed) newAllowedValues = newAllowedValues.concat(rule.value);
			break;
		case RuleName.ALLOWED:
	...
	if (newAllowedValues.length) newPasswordRules.push(new Rule(RuleName.ALLOWED, newAllowedValues));
	if (minimumMaximumConsecutiveCharacters !== null) newPasswordRules.push(new Rule(RuleName.MAX_CONSECUTIVE, minimumMaximumConsecutiveCharacters));
	if (maximumMinLength > 0) newPasswordRules.push(new Rule(RuleName.MIN_LENGTH, maximumMinLength));
	if (minimumMaxLength !== null) newPasswordRules.push(new Rule(RuleName.MAX_LENGTH, minimumMaxLength));
